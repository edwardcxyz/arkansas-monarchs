...
export default {
  ...
  computed: {
    ...
    plantsBySeason() {
        let spring = this.plants.filter(p => p.bloomPeriod === 'Spring');
        let springSummer = this.plants.filter(p => p.bloomPeriod === 'Spring to Summer');
        let summer = this.plants.filter(p => p.bloomPeriod === 'Summer');
        let summerFall = this.plants.filter(p => p.bloomPeriod === 'Summer to Fall');
        let fall = this.plants.filter(p => p.bloomPeriod === 'Fall');
        let result;
        switch (this.season) {
            case 'Spring': result = spring; break;
            case 'Spring to Summer': result = springSummer; break;
            case 'Summer': result = summer; break;
            case 'Summer to Fall': result = summerFall; break;
            case 'Fall': result = fall; break;
            default: result = [
                    ...spring,
                    ...springSummer,
                    ...summer,
